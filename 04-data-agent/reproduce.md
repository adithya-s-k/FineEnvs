...
Job outputs contain `status.json`, `training_recipe.json`, `space_identity.json`, `training_smoke_verified.json`, `canonical_scores.json`, capture audits and native checkpoints. Remote artifacts are stored under the run ID and unique job owner. Checkpoints publish their ready marker only after all files and hashes are verified. Local source transformations have their own manifest; the original portable bundle remains intact.
