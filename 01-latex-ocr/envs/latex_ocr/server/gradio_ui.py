...
def latex_ocr_ui_builder(
    ...
    with gr.Blocks(title=f"{title} — Playground") as demo:
        ...
        gr.Markdown(
            ...
            f"`{1 - web_manager.env._rubric.exact_weight:g}·(1−CER) + "
            f"{web_manager.env._rubric.exact_weight:g}·exact_match`, multiplied by "
            "a length guard against padded completions, against the hidden ground truth."
        )
        ...
        async def on_score(prediction: str):
            ...
            except Exception as e:  # noqa: BLE001 - present server errors in the playground
