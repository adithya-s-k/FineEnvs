...
def _load_split(dataset_name: str, split: str):
    ...
    cap = _max_rows()
    logger.info("Loading %s split=%s cap=%s ...", dataset_name, split, cap)
    if cap is not None:
        if cap < 0:
            raise ValueError("LATEX_OCR_MAX_ROWS must be nonnegative")
        # Cap before materializing: selecting after load_dataset downloads the
        # complete split, even for a two-row smoke test.
        rows = load_dataset(
            dataset_name,
            name=CONFIG,
            split=split,
        ...
        return Dataset.from_list(list(islice(rows, cap)))
    return load_dataset(
...
class LatexOCREnvironment(Environment):
    ...
    def __init__(
        ...
        if mode not in {"stream", "materialize"}:
            raise ValueError("mode must be materialize or stream")
        self.mode = mode
    ...
    def _reset_stream(self, split: str) -> LatexOCRObservation:
        ...
        self._target = str(row[TEXT_COLUMN])
        self._done = False
        self._current_split, self._current_index = split, self._cursor - 1
        self._current_task_id = f"{split}-{self._cursor - 1}"
        ...
        return LatexOCRObservation(
            ...
            task_id=f"{split}-{self._cursor - 1}",
            total=total,
            remaining=remaining,
