...
def test_failed_reset_invalidates_previous_target(monkeypatch):
    monkeypatch.setattr(
        environment_module,
        "_load_split",
        lambda *_: [
    ...
    monkeypatch.setattr(environment_module, "_encode_image", lambda _: "png")
    env = LatexOCREnvironment()
    env.reset(index=0)
    with pytest.raises(IndexError):
        env.reset(index=10)
    with pytest.raises(RuntimeError, match="before reset"):
        env.step(environment_module.LatexOCRAction(latex="x^2"))


def test_stream_indices_match_task_discovery(monkeypatch):
    monkeypatch.setattr(environment_module, "_split_total", lambda *_: 2)
    monkeypatch.setattr(environment_module, "_encode_image", lambda _: "png")
    env = LatexOCREnvironment(mode="stream")
    env._stream = iter([{"image": None, "text": "x"}, {"image": None, "text": "y"}])
    env._stream_split = "train"
    for index in range(2):
        observation = env.reset()
        assert observation.index == index
        assert observation.task_id == env.get_task("train", index)["id"]
        assert observation.pct_done == (index + 1) / 2
        assert (
            env.step(environment_module.LatexOCRAction(latex="x")).task_id
            == observation.task_id
        )
    assert env.reset().exhausted
    with pytest.raises(RuntimeError):
        env.step(environment_module.LatexOCRAction(latex="y"))


def test_materialize_cap_only_consumes_requested_rows(monkeypatch):
    import datasets
    ...
    monkeypatch.setenv("LATEX_OCR_MAX_ROWS", "2")
    environment_module._load_split.cache_clear()
    try:
        assert len(environment_module._load_split("fixture", "train")) == 2
        assert consumed == [0, 1]
    finally:
        environment_module._load_split.cache_clear()
