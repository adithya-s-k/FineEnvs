"""Typed client. Mirrors the OCR environment so a multi-environment runner reads alike."""
...
class AsrClient(EnvClient[AsrAction, AsrObservation, State]):
    ...
    def _parse_result(self, data):
        ...
        return StepResult(
            observation=AsrObservation(**observation),
    ...
    def manifest(self):
        response = requests.get(f"{self._http_base()}/manifest", timeout=60)
...
def connect(url):
    return AsrClient(base_url=url, connect_timeout_s=60, message_timeout_s=180).sync()
