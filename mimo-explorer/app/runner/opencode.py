...
def config_for(model: str, provider: str | None, steps: int, mcp: list[dict] | None = None) -> dict:
...
def run(r, prompt: str, cwd: str, *, steps: int, timeout: float, user: str | None = None,
    ...
    cfg = config_for(r.run["model"], r.run.get("provider"), steps, mcp)
    ...
    env = f"env HF_TOKEN=\"$HF_TOKEN\" OPENCODE_CONFIG=/tmp/opencode.json HOME={home or '$HOME'}"
    inner = f'{env} opencode run --format json --auto "$(cat /tmp/prompt.txt)"'
